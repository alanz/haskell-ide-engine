    ...
    it "deals with cabal file with unsatisfiable dependency" $
      ...
      -- runSessionWithConfig logConfig hieCommandExamplePlugin codeActionSupportCaps "test/testdata" $ do
        ...
        liftIO $ do
          ...
          d ^. message `shouldBe`
            (T.pack "callProcessStderr: stack --stack-yaml=/Users/luke/Source/haskell-ide-engine/test/testdata/badProjects/cabal/stack.yaml build --only-configure . (exit 1): failed\n")
